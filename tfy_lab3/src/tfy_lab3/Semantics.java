...
public class Semantics {
	...
	boolean checkTypeAndThrowErrors(char[] typeId, Types lex) {
		...
		if (lex == Types.Tid) {
			...
			if (!typeOk) {
				throwError("Ќеверный тип " + new String(typeId).trim());
			}
		} else {
			typeOk = checkRawType(lex);
			
			if (!typeOk) {
				throwError("Ќеверный тип " + new String(lex.toString()).trim());
			}
		}
	...
	boolean checkVarAndThrowErrors(char[] id) {		
		if (findVar(id) != null) {
			...
			throwError("Ќеверна€ переменна€ " + new String(id).trim());
			return false;
		}
	}
	...
	RefType addRefType(char[] typeId, char[] refTypeId, Types refRawType, int textPos) {
		if (findRefType(typeId) != null) {
			throwError("“ип " + new String(typeId).trim() + " уже есть");
			return null;
		}
	...
	void addVar(char[] typeId, Types type, char[] id, long value, int textPos) {
		...
		if (findVar(id) != null) {
			throwError("ѕеременна€ уже есть " + new String(id).trim());
			return;
		}
...
//		values.add(val);
	...
	void returnFromBlock() {
		...
		if (currNode == null) {
			throwError("«акрываетс€ неоткрытый блок");
			return;
		}
