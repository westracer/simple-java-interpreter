...
public class Semantics {
	...
	boolean checkVarLength(char[] varId, Integer[] indices) {
		...
		if (indicesLength > varArrayLength) {
			throwError("Неверная размерность переменной " + new String(varId).trim() + ": " + indicesLength + " > " + varArrayLength);
			return false;
		} else if (indicesLength < varArrayLength) {
			throwError("Операции над массивами не разрешены для " + new String(varId).trim() + ": " + indicesLength + " < " + varArrayLength);
			return false;
		}
